...
\title{SmartPic: Bridging User Interest and Product Content with Hybrid Machine Learning for E-Commerce Platforms}
...
\subsubsection{Hybrid Recommendation Engine}
A real-time hybrid engine was developed to combine collaborative and content-based scores using a weighted combination formula:
\begin{equation}
Hybrid\_Score = \alpha \cdot CF\_Score + (1 - \alpha) \cdot CBF\_Score
\end{equation}
where $\alpha$ is a configurable weight (default 0.5), $CF\_Score$ is the normalized predicted rating from the KNN model, and $CBF\_Score$ is the cosine similarity between TF-IDF product vectors.
...
\begin{enumerate}
    \item \textbf{Deep Learning:} Implement neural collaborative filtering (NCF)
    \item \textbf{Real-time Learning:} Online model updates based on live user streams
    \item \textbf{Multi-Modal Data:} Incorporate image-based similarity using Computer Vision
    \item \textbf{Context-Aware Recommendations:} Time-decay and session-based personalization
\end{enumerate}
